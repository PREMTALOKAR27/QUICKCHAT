...
const ProfilePage = () => {
  ...
  return (
    <div className="min-h-screen bg-cover flex items-center justify-center">
      ...
      flex items-center justify-between max-sm:flex-cool-reverse rounded-lg">
        <form onSubmit={(e)=>handleSubmit(e)} className="flex flex-col gap-5 p-10 flex-1">
          <ChevronLeft onClick={()=>navigate('/')} className="self-end block sm:hidden "/>
          ...
          <textarea onChange={(e)=> setBio(e.target.value)} value={bio} role="{4}" placeholder="Write profile bio" required
            className="p-2 border border-gray-500 rounded-md focus:outline-none focus:ring-2 focus:ring-violet-500">
          </textarea>

          <button type="submit" className="bg-gradient-to-r from-purple-400 to-violet-600 text-white p-2 rounded-full text-lg cursor-pointer">Save</button>
        </form>
        <div className="flex flex-col h-100 hidden sm:block">
          <div className="flex justify-end p-2">
            <ChevronLeft onClick={()=>navigate('/')}/>
          </div>
          <div className="h-90 flex justify-center">
            <img className={`max-w-44 aspect-square rounded-full mx-10 max-sm:mt-10 ${selectedImg && 'rounded-full'}`} src={authUser.profilePic ||assets.favicon} alt="" />
          </div>
