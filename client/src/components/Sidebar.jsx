...
import { EllipsisVertical,Search, SpaceIcon } from 'lucide-react'
      ...
      <div className='flex flex-col'>
        {userDummyData.map((user,index)=>(
          <div onClick={()=>{setSelecteduser(user)}} key={index} className={`relative flex items-center gap-2 p-2 pl-4 rounded cursor-pointer max-sm:text-sm ${selectedUser?._id === user._id && 'bg-[#282142]/50'}`}> 
            ...
            {index>2 && <p className='absolute flex justify-center item-center top-4 right-2 h-6 w-5 rounded-full bg-violet-500/50' >{index}</p>}
          </div>
        ))}
      </div>
    </div>
